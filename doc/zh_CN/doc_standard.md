...
仓库文档只描述**当前分支中的真实实现**。截至 `2026-07-11`，当前基线为
**`0.4.0`**。
...
- 按 MoonBit 包组织正文。文件名不会创建 MoonBit 模块，包边界由 `moon.pkg` 决定。
...
- 明确区分 `internal`、CLI、测试和一致性基础设施与稳定应用接口。
...
- `*_result` API 必须说明短路行为，以及值变换组合与 observer 结果之间的区别。
